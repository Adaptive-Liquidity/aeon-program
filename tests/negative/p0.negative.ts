...
describe("P0 negative e2e", function () {
  ...
  describe("NEG-AUTH hierarchy", () => {
    ...
    it("NEG-AUTH-005 ParentRequired when parent_id≠0 and parent null", async () => {
      ...
      const agentAClient = createAgentClient(fx.agentA, aeon);
      await expectAeonError(
        agentAClient.issueAuthority({
          authorityId: next,
          budget: 5 * ONE,
          maxPerTx: 5 * ONE,
          maxTotal: 5 * ONE,
    ...
    it("NEG-AUTH-007 cross-agent parent → Unauthorized", async () => {
      ...
      const next = (await aeon.nextIds()).authorityId;
      const agentBClient = createAgentClient(fx.agentB, aeon);
      await expectAeonError(
        agentBClient.issueAuthority({
          authorityId: next,
          budget: 5 * ONE,
          maxPerTx: 5 * ONE,
          maxTotal: 5 * ONE,
  ...
  describe("NEG-PAY spend gates", () => {
    ...
    it("NEG-PAY-002b InsufficientBudget when ATA balance too low", async () => {
      const idB = await (async () => {
        const next = (await aeon.nextIds()).authorityId;
        const agentBClient = createAgentClient(fx.agentB, aeon);
        await agentBClient.issueAuthority({
          authorityId: next,
          budget: 1000 * ONE,
          maxPerTx: 500 * ONE,
          maxTotal: 1000 * ONE,
